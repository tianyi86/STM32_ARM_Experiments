...
void CAN1_Init(void) {
    ...
    //CAN时序配置 500Kbps LoopBack自发自收
    CAN_InitStruct.CAN_Prescaler = 6;       //分频系数
    CAN_InitStruct.CAN_Mode = CAN_Mode_LoopBack;     //自发自收模式 
    CAN_InitStruct.CAN_SJW  = CAN_SJW_1tq;    //同步跳跃宽度
    CAN_InitStruct.CAN_BS1  = CAN_BS1_6tq;  //相位缓冲段1
    CAN_InitStruct.CAN_BS2  = CAN_BS2_7tq;  //相位缓冲段2
...
void CAN1_FilterConfig(void){
    ...
    CAN_Filter.CAN_FilterScale      = CAN_FilterScale_32bit; //32位过滤器
    CAN_Filter.CAN_FilterIdHigh     = 0x0000;
    CAN_Filter.CAN_FilterIdLow      = 0x0000;
    CAN_Filter.CAN_FilterMaskIdHigh = 0x0000;//掩码全0，接收所有ID
    CAN_Filter.CAN_FilterMaskIdLow  = 0x0000;
    CAN_Filter.CAN_FilterFIFOAssignment = CAN_Filter_FIFO0;//过滤器0关联FIFO0
...
void CAN1_RX0_IRQHandler(void){
    ...
    CAN_Receive(CAN1, 0, &RxMessage);//读取FIFO0接收数据
...
void CAN1_SendMsg(uint32_t uID, uint8_t *pData, uint32_t uLen){
    ...
    TxMessage.StdId = uID;     // 标准标识符
    TxMessage.ExtId = uID;     // 扩展标示符（29位）
    TxMessage.IDE   = CAN_Id_Extended;    // 使用扩展标识符
    TxMessage.RTR   = CAN_RTR_Data;          // 消息类型为数据帧
    TxMessage.DLC   = uLen;             // 数据长度
