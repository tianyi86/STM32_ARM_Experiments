...
/*
检验返回的数据是否合规（自动跳过ACK定位Response帧）
搜索00 00 FF前导码，跳过LEN=0的ACK帧，定位真正的Response
返回0=成功（resp_off指向Response帧起始），-1=格式错误
*/
...
len = PN532_RxBuf[off + INDEX_LEN];//帧长
temp = 0x100 - len;//计算LCS值
if(temp != PN532_RxBuf[off + INDEX_LEN + 1]){return -1;} //LCS不匹配
...
temp = 0x100 - temp;
if(temp != PN532_RxBuf[off + INDEX_TFI + len] ){ return -1;}//校检码不匹配
...
if(PN532_CheckRxBuf(&off)<0){ ret = -1; goto _END; }//返回数据格式不对
ret = 0;
...
if(PN532_CheckRxBuf(&off)<0){ret = -1;goto _END;}//返回数据不正确
if(PN532_RxBuf[off + INDEX_TFI + 2] != 0){ ret= -1; goto _END;}//status状态位不正确
...
memcpy(buf, PN532_RxBuf + off + INDEX_TFI + 3, 16); //读数据
ret = 0;
