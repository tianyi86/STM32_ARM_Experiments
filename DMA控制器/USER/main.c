...
int main(void)
{
    ...
    DMA_InitStruct.DMA_PeripheralBaseAddr = (u32)&USART2->DR;//外设地址串口2DR寄存器
    DMA_InitStruct.DMA_Memory0BaseAddr = (u32)SendBuf;//内存数组首地址
    DMA_InitStruct.DMA_DIR = DMA_DIR_MemoryToPeripheral;//内存→外设
    DMA_InitStruct.DMA_BufferSize = 17;//传输17字节
